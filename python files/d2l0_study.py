...
for j in range(lowest_low_order , highest_low_order):
    ...
    for i in range(lowest_high_Order, highest_high_order):
        ...
        for k in range(l):
            ...
            H_h = Compress(H1(mesh, order = j, complex=True)) # main function space
...
#np.save('d{d}l{l}_minEV'.format(d=2,l=0),minEV)
np.save('/cluster/home/rschoenholze/Bsc_Thesis/data/d{d}l{l}_minEV'.format(d=2,l=0),minEV)
...
for j in range(lowest_low_order, highest_low_order):
    ...
    plt.loglog(meshwidths,np.ones(l) * 0.5,'--k', label="0.5")
...
for j in range(lowest_low_order, highest_low_order):
    ...
    plt.title(r"convergence to $\lambda_{ref}=0.5$, low order=%i" %j) 
    ...
    plt.legend()
    #plt.show()
    #plt.savefig("../plots/higherOrders/d2l0/d2l0_convergence_theory_o%i.pdf" %j)
    plt.savefig("/cluster/home/rschoenholze/Bsc_Thesis/higherOrders/d2l0/d2l0_convergence_theory_o%i.pdf" %j)
...
for j in range(lowest_low_order, highest_low_order):
    ...
    plt.legend()
    #plt.show()
    #plt.savefig("../plots/higherOrders/d2l0/d2l0_convergence_FineEV_o%i.pdf" %j)
    plt.savefig("/cluster/home/rschoenholze/Bsc_Thesis/higherOrders/d2l0/d2l0_convergence_FineEV_o%i.pdf" %j)